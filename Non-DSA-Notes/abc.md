...
* **Polymorphism :** allows the same interface (method/function) to behave differently depending on the object that invokes it. Same method. Different behavior.
...
**1) Method Overriding :** In object-oriented programming, method overriding happens when a child class provides a specific implementation for a method that is already defined in its parent class


**2) Duck Typing :** Python is a dynamically typed language, meaning it cares about what an object can do, not what it is. This follows the phrase: "If it walks like a duck and quacks like a duck, it's a duck." If an object has the required methods, it works.
...
* Q : (Interview Favorite) Suppose you're building a notification service. You have: * Email, * SMS, * Push Notification.
