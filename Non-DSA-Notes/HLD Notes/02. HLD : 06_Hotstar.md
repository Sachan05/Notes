...
**1. Video File Storage and Chunking :**
Raw Video Upload: When a raw video file is uploaded, it is typically divided into chunks rather than being stored as a single large file. This chunking facilitates efficient storage and retrieval, allowing parallel processing and reduced latency during viewing.

Chunk Size Considerations: The optimal chunk size is crucial for performance. While a uniform chunk size might be used in general systems, services like Netflix may determine their chunk sizes based on extensive data analysis and specific use-case requirements

**2. Data Ingestion Pipeline**
Initial Steps: This includes uploading the raw video, receiving metadata (such as timestamps, actor information, etc.), and then preparing the video for further processing.

Processing Steps: Once the raw video is uploaded, the data ingestion pipeline triggers various processes such as format conversion (MP4, MKV, etc.), resolution adjustments (360p, 720p, etc.), and compression. These processes are designed to happen in parallel to increase efficiency.

**3. Format and Resolution Conversion**
Parallel Processing: Videos are converted into multiple formats and resolutions to cater to different devices and network conditions. This conversion uses parallel processing whereby independent consumer groups handle different tasks (e.g., converting to MP4 and MKV simultaneously).

**4. Compression and Optimization**
Codexplained as Compression: To optimize for bandwidth and storage, videos are compressed. This involves storing only changes between frames rather than the entire frames, drastically reducing file size without significant loss in quality.

**5. Delivery via CDN (Content Delivery Network)**
Efficient Fetching: CDNs play a critical role in delivering video content efficiently. Logical chunks and metadata are stored with CDN URLs to facilitate quick access to specific video segments without fetching all previous chunks, which minimizes latency and improves user experience.

**6. Adaptive Bitrate Streaming (ABS)**
Real-time Adjustment: ABS dynamically adjusts the quality of a video stream in real-time, based on the user's current bandwidth and device capabilities, ensuring smooth playback without interruptions.

**7. Message Queues and Events**
Handling Video Processing Events: Message queues like Kafka are used extensively to manage events related to video processing and format conversion, enabling scalable and efficient data pipelines.

**8. Storage Strategy**
Replication and Backup: Raw and processed videos may be stored with multiple replicas to ensure data durability and availability.
...
* Features :
  ...
  - Ingestion of Data
  - Movies/videos must be available in different qualities (depending on network for smooth user experience)
 
* Movies will be stored in file storage systems like S3
  
* Tradeoff :
  - Avalaibility >> Consistency
  - Low Latency
 
* Estimate Raw videos size stored in OTT platforms : (which is late further optimized)
  - A video is set of Images (360p, 480p, 720p, 1080p, 2k, 4k)
  - each image is grid of pixels, each pixel can be defined by RGB(3 bytes). assuming 1080x1920 grid.
  - assuming 3 hours long video of 60FPS(frames per second) and 1080p qulaity.
  - 60 * 60 * 60 * 3 * 1080 * 1920 * 3 bytes ~ 600 GB (Raw video)
 
* Codex : compression and Decompression, multiple formats (mp4, mkv, mov...)

* ABS : Adaptive Bitrate Streaming
  Clients are going to have different internet speed, different devices, and internet speed can also change over time. Based on these pointers our platform should suggest the right type of video to the user. vidoes need to be stored in different resolutions(260p, 480p,..)


* Data Ingestion pipeline :
  - Raw video + metadata : 1-2TB
  - once the raw video is stored is uploaded to file storage systems(HDFS, S3), data ingestion pipeline gets triggered.
  - it's passed to message queues along with the s3 url of the video, where it's consumed by the different conversion services in the consumer groups (mp4, mkv, mov). after conversion its reuploaded to S3 with converted format.
  - It's then passed to another Resolution queue with the converted format S3 URL where its further processed to different resolutions.
  - its stored in a table with each video id, its different formats and resolutions which is processed according to ABS.
